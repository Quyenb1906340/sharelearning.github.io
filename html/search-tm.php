...
if (isset($_GET['noidung']) && !empty($_GET['noidung'])) {
    ...
    foreach ($resultArray as $doc_id => $cosineSimilarity) {
        if ($cosineSimilarity > 0) {
            ...
            kd.tt_ma = 1  AND (kd.ghi_chu != 5 OR kd.ghi_chu IS NULL) AND  bv.bv_ma = $doc_id GROUP BY  bv.bv_ma";
...
JOIN khoi_lop kl on kl.kl_ma  = mh.kl_ma
LEFT JOIN kiem_duyet kd on kd.bv_ma = bv.bv_ma
WHERE kd.tt_ma = 1   AND (kd.ghi_chu != 5 OR kd.ghi_chu IS NULL) -- Join the mon_hoc table
GROUP BY bv.bv_ma
