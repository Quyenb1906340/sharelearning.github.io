...
if (mysqli_num_rows($result_bv) > 0) {
    ...
    echo "Không có bài viết chờ duyệt!";
}
...
$adminPages = array("QL_DanhMuc.php", "QL_KhoiLop.php", "QL_MonHoc.php", "QL_NguoiDung.php",  "PCQL_DanhMuc.php");
...
elseif($_SESSION['vaitro'] == 'Admin'){
    ...
    LEFT JOIN kiem_duyet k ON k.bv_ma = a.bv_ma
    LEFT JOIN trang_thai t ON t.tt_ma = k.tt_ma
    LEFT JOIN quan_ly g ON g.dm_ma=a.dm_ma
    WHERE (k.tt_ma is null or k.tt_ma = 3)
    And g.nd_username = '$user_name'
    And DATE(bv_ngaydang) BETWEEN '$tungay' AND '$denngay'
            ORDER BY a.bv_ngaydang $sap_xep LIMIT $so_dong";
} 
                                ...
                                <div class="modal-dialog modal-xl" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="text-center ms-4 me-4">
                                            <h5 class="modal-title text-center" id="modalCenterTitle">CHI TIẾT KIỂM TRA
                                            </h5>
                                        ...
                                        <form method="POST" enctype="multipart/form-data">
                                            <div class="ms-4 me-4">
                                                <button name="huybai" type="submit" class="btn btn-outline-secondary"
                                                    data-bs-dismiss="modal">Từ chối viết

                                                </button>
                                                <button name="duyetbai" type="submit" class="btn btn-primary">Duyệt
                                                    bài</button>
                                            </div>
                                    ...
                                    <table class="table" id="dataTable">
                                        ...
                                        <tbody class="table-border-bottom-0" id="data-container">
                                            ...
                                            <tr>
                                                ...
                                                <td style="white-space: normal"> <?php echo $row_bai_viet['nd_hoten'] ?>
                                                </td>
                                                <td style="white-space: normal">
                                                    <?php
                                                ...
                                                <?php
                                                    ...
                                                    <?php }
                                                                ...
                                                                class="dt-button buttons-print dropdown-item"
                                                                tabindex="0" type="button">
                                                                <span><i class=" fa fa-eye me-2"></i></span>
                                                            </a>
                                                               <?php } 
                                                            ?>
        ...
        $('#loc_ngay').click(function() {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    sodong: chon_gtri,
                    denngay: denngay,
                    similarRows: JSON.stringify(
                    <?php echo json_encode($similarRows); ?>), // Chuyển đổi thành chuỗi JSON
...
<script>
    ...
    function removeVietnameseAccent(str) {
        // Sử dụng phương pháp normalize để chuẩn hóa chuỗi Unicode về dạng "Canonical Decomposition"
        // và sau đó loại bỏ các ký tự dấu (combining diacritical marks)
        return str.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    }

    // Lấy tham chiếu đến input dùng để nhập từ khóa tìm kiếm
    const searchInput = document.getElementById('searchInput');

    // Lấy tham chiếu đến bảng hoặc thành phần chứa dữ liệu cần tìm kiếm
    const dataTable = document.getElementById('dataTable');

    // Lấy tất cả các hàng (tr) trong bảng
    const rows = dataTable.getElementsByTagName('tr');

    // Thêm sự kiện 'keyup' để tìm kiếm mỗi khi người dùng gõ phím
    searchInput.addEventListener('keyup', function() {
        // Lấy giá trị từ khóa tìm kiếm và chuyển thành chữ thường
        const searchText = removeVietnameseAccent(searchInput.value.toLowerCase());

        // Duyệt qua từng hàng trong bảng, bắt đầu từ hàng thứ hai (vì hàng đầu tiên thường là tiêu đề)
        for (let i = 1; i < rows.length; i++) {
            // Lấy tất cả các ô (td) trong từng hàng
            const cells = rows[i].getElementsByTagName('td');
            let found = false;

            // Duyệt qua từng ô trong hàng
            for (let j = 0; j < cells.length; j++) {
                // Lấy nội dung của ô và loại bỏ dấu tiếng Việt, sau đó chuyển thành chữ thường
                const cellText = removeVietnameseAccent(cells[j].textContent.toLowerCase());

                // Kiểm tra xem từ khóa tìm kiếm có tồn tại trong ô hay không
                if (cellText.includes(searchText)) {
                    found = true; // Nếu có, đánh dấu là đã tìm thấy
            ...
            if (found) {
                rows[i].style.display = ''; // Hiển thị hàng nếu tìm thấy
            } else {
                rows[i].style.display = 'none'; // Ẩn hàng nếu không tìm thấy
            }
        }
    });
    </script>
