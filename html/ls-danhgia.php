                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="row">
                                    ...
                                    <div class="position-fixed top-0 end-0" style="z-index: 11; margin-top:75px">
                                                       <div id="bg_thongbao_nd" class="toast hide <?php echo $_SESSION['bg_thongbao_nd']; ?>" role="alert"
                                                            aria-live="assertive" aria-atomic="true">
                                                            <div class="toast-header">
                                                                 <!-- <span><i class="fa-solid fa-circle-check fa-lg"> </i> &nbsp; </span> -->
                                                                 <strong class="me-auto"><?php echo $_SESSION['thongbao_thucthi_nd']; ?></strong>
                          ...
                          <li class="breadcrumb-item">
                              <a href="profile.php?id=<?php echo $id ?>"><?php echo 'Hồ sơ' ?></a>
                            </li>
                            <li class="breadcrumb-item">
                              <a href="nhatkyhoatdong.php?id=<?php echo $id ?>"><?php echo 'Nhật ký hoạt động' ?></a>
                            </li>
                            <li class="breadcrumb-item active"><?php echo 'Lịch sử đánh giá'?></li>
                          </ol>
                        </nav>
                                  ...
                                  <div class="row card-header d-flex flex-wrap py-3  justify-content-between">
                                    
                                    <div style="margin-bottom:1rem; margin-top:-2rem" class="col-lg-12  row">
                                  ...
                                  <?php
                                  $username = $_GET['id'];
                                  $nguoi_dung = "SELECT dg.*, nd.nd_username as chunhan, nd_hinh, nd_hoten FROM danh_gia dg , bai_viet bv, nguoi_dung nd 
                                  WHERE dg.bv_ma = bv.bv_ma and bv.nd_username = nd.nd_username and dg.nd_username='$username' Order by dg_thoigian desc";
                                  ...
                                  while ($row = mysqli_fetch_assoc($result_nguoi_dung)) {
                                    $ls_thoigian = $row['dg_thoigian']; // Assuming ls_thoigian contains a date/time
                                  ...
                                  if (mysqli_num_rows($result_nguoi_dung) > 0) {
                                    foreach ($grouped_articles as $date => $articles) {
                                      ...
                                      foreach ($articles as $key => $article) {
                                        ...
                                        echo '<h6 class="mb-0 text-dark">Bạn đã đánh giá '.$article['dg_diem'].'<i class="fa-solid fa-star" style="color: #FDCC0D;"></i> cho ';
                                        ...
                                        echo '<small>' . date("H:i:s", strtotime($article['dg_thoigian'])) . '</small>';
                                        echo '</div>';
                                        echo '</div>';

                                        // Add ellipsis icon
                                        echo '<div class="col-1 mb-sm-0">';
                                        // echo '<a id="dropdownHanhDong" data-bs-toggle="dropdown" aria-expanded="false" style="display:math; padding:0.1rem 0.6rem" class="dropdown-item" href="Xem_BinhLuan.php?this_dg_ma=' . $article['dg_ma'] . '&tg=' . $article['nd_username'] . '">';
                                        ...
                                        echo '        <a href="#" onclick="Xoa_danhgia('. $article['dg_ma'] .','. $article['bv_ma'] .');" type="button">';
                                        echo '            <span><i style="color:red" class="bx bx-trash me-2"></i></span>';
                                        echo '        </a>';
                                    ...
                                    echo 'Không có lịch sử đánh giá';
                                  }
...
<script>
        function Xoa_danhgia(dg_ma, bv_ma) {
            Swal.fire({
                title: 'Xác nhận xóa',
                ...
                if (result.isConfirmed) {
                    ...
                    window.location.href = 'xoa_danhgia.php?dg_ma=' + dg_ma +  '&bv_ma=' +bv_ma;
                }
            });
        }
    </script>
...
<script>
    <?php if (isset($_SESSION['thongbao_thucthi_nd']) && $_SESSION['thongbao_thucthi_nd']) { ?>
    document.addEventListener("DOMContentLoaded", function() {
        const successToast = document.getElementById("bg_thongbao_nd");
        var successToastInstance = new bootstrap.Toast(successToast);
        $("#loader").hide();
        successToastInstance.show();
    });
    <?php
      ...
      unset($_SESSION['thongbao_thucthi_nd']);
      unset($_SESSION['bg_thongbao_nd']);
    }
    ?>
    </script>
